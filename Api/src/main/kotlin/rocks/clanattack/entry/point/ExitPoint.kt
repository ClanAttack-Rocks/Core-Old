package rocks.clanattack.entry.point

import rocks.clanattack.entry.Registry
import rocks.clanattack.entry.service.Service

/**
 * A [ExitPoint] is the end of the execution of a plugin.
 *
 * For a method to be called as an exit point, it must be annotated.
 *
 * The method must be  in a class eligible for an automatic [Registry.create] call **and** return nothing.
 *
 * The method can be private, but must be accessible.
 *
 * The method can be suspending, but if it is suspending,
 * it will be executed asynchronously.
 *
 * Every method will be called before any [Service] is disabled.
 *
 * ```
 * @ExitPoint
 * fun onDisable() {
 *    // This method will be called when the plugin is disabled.
 * }
 * ```
 */
annotation class ExitPoint
